...
def main():
    ...
    sql_create_run_table = """CREATE TABLE IF NOT EXISTS Run (number TINYINT PRIMARY KEY, date DATETIME, 
    ...
    sql_create_report_table = """CREATE TABLE IF NOT EXISTS Responded (empNumber STRING REFERENCES Employee (number), 
                                runNumber TINYINT REFERENCES Run (number), date DATETIMEdatetime REFERENCES Run (date), 
                                payRate FLOAT);"""
